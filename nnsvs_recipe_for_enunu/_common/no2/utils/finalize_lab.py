    ...
    if config["force_sil_pau_suppress"]:
        ...
        for i in range(len(lab)):
            if lab.contexts[i] == "sil" or lab.contexts[i] == "pau":
                ...
                end_idx = int(lab.end_times[i] * 1e-7 * config["sample_rate"])
                print(f"{lab.contexts[i]} is suppressed from {lab.start_times[i] * 1e-7} ({start_idx}) to {lab.end_times[i] * 1e-7} ({end_idx}).")
